...
try:
    ...
    from modules import hotwordDetection
...
class gravia:
    def __init__(self, parent):
        ...
        self.timeLabel = Label(top_menu_frame,text=datetime.datetime.now().strftime("%c"), bg="white",fg="green", font="Digital-7 20")
        self.timeLabel.pack(side=RIGHT,pady=2)
        self.timeLabel.after(1000, lambda : Thread(target=self.clock).start())

        # Statusbar
        self.statusbar_frame = Frame(self.root, borderwidth=1, relief=RIDGE, bg="white")
    ...
    def speak(self, audio, wordwrap=True):
        ...
        try:
            ...
            self.engine.runAndWait()
            self.engine.endLoop()
        # Thread(target=lambda: self.mainspeakfun(audio)).start()
    ...
    def opensetting(self):
        setting_window = Tk()
        setting_window.geometry('1000x600')
        setting_window.title("Setting")

        slabel = Label(setting_window,text="Setting will comming soon...", font="Helvetica 40 bold")
        slabel.pack(side=BOTTOM,pady=100)
        slabel.after(1000*3,setting_window.destroy)
    ...
    def menu(self, e=None):
        ...
        menuWindow.config(bg="white",cursor="hand2",borderwidth=2, highlightbackground="black", highlightthickness=2)
...
if __name__ == '__main__':
    with open("login.txt", 'r') as logedin:
        ld = logedin.read()
        ld = int(ld)
        if ld == 0:
            win = tix.Tk()
            app = login.Login_Window(win,"Gravia")
            win.mainloop()
            if app.logedinVar.get() == "Yes":
                ...
                root.title('Gravia')
                root.iconify()
                root.iconbitmap(r"static\Icons\Gravia icon.ico")
