...
class Login_Window():
    def __init__(self,root,appname) -> None:
        ...
        self.mydb = self.myclient[self.appname]
        ...
        # Background image
        self.bg = ImageTk.PhotoImage(file="static\\backgrounds\\0.jpg")
        ...
        img1 = PIL.Image.open("static\\avatar.png")
        img1 = img1.resize((100,100),PIL.Image.ANTIALIAS)
        self.phtoimage1 = ImageTk.PhotoImage(img1)
        Label(self.root,image=self.phtoimage1,bg="black",bd=0).place(x=260,y=85)
        ...
        img2 = PIL.Image.open("static\\avatar.png")
        img2 = img2.resize((25,25),PIL.Image.ANTIALIAS)
        self.phtoimage2 = ImageTk.PhotoImage(img2)
        Label(image=self.phtoimage2,bg="black",bd=0).place(x=175,y=240)

        img3 = PIL.Image.open("static\\lock.png")
        img3 = img3.resize((25,25),PIL.Image.ANTIALIAS)
        self.phtoimage3 = ImageTk.PhotoImage(img3)
        Label(image=self.phtoimage3,bg="black",bd=0).place(x=175,y=330)

        img4 = PIL.Image.open("static\\close eye.png")
        img4 = img4.resize((30,25),PIL.Image.ANTIALIAS)
        self.hide_image = ImageTk.PhotoImage(img4)

        img5 = PIL.Image.open("static\\open eye.png")
        ...
        self.show_button.place(x=275,y=282)
        ...
        forgotten_pass_btn.place(x=20,y=412) 
            ...
            if mydoc != None:
                if bcrypt.checkpw(encoded_password, mydoc["password"]):
                    ...
                    notification.notify(
                        app_name = "Gravia",
                        title = "Successfull",
                        message = f"Congratulations you successfully logedin in {self.appname}",
    ...
    def openRegisterWindow(self):
        ...
        self.app = Signup(new_window, self.appname)
...
class Signup:
    def __init__(self, root, appname) -> None:
        ...
        self.bg = ImageTk.PhotoImage(PIL.Image.open(f"static\\backgrounds\\{images[0]}"))
        ...
        def change():
            if self.count == len(images) - 1:
                ...
                self.bg = ImageTk.PhotoImage(
                    PIL.Image.open(
                        f"static\\backgrounds\\{images[self.count + 1]}")
                )
                background_label.config(image=self.bg)
                background_label.update()
                self.count += 1
        ...
        frame.place(x=280, y=120, width=950, height=550)
        ...
        self.show_image.image = self.show_image
        self.show_button.place(x=750, y=390)
        self.tooltip.bind_widget(self.show_button, msg="Show")
    ...
    def signup(self):
        ...
        try:
            validatePhonenumber = validator.validatePhonenumber.check(phone)
        ...
        if t == 0:
            ...
            otp_window = Tk()    
            otp = generateOTP(otp_window, e, fname, self.register)   
            otp_window.mainloop()
    ...
    def register(self):
        ...
        if len(mylist) == 0:
            ...
            notification.notify(
                title="Sucesfully",
                message=f"You are succesfully signed up in {self.appname}",
            )
        else:
            tmsg.showerror(
                "Error",
