...
"""______________________________________________________________Rounded Buttons________________________________________________________-
from tkinter import *
...
button = RoundedButton(root, 100, 50, 20, 2, 'red', 'white', command=test)
button.place(relx=.1, rely=.1)

root.mainloop()
"""

# def listencancel(self):
...
#     # global query
#     while self.listen.get() == True:
#         # self.listenButton.config(command=self.listencancel)
#         self.statusvar.set("Listening...")
#         winsound.PlaySound(r"static\sound clips\Click.wav",winsound.SND_FILENAME)
#         r = sr.Recognizer()
#         with sr.Microphone() as source:
#             # r.adjust_for_ambient_noise(source, duration=0.8)   # For removing background noise
#             audio = r.listen(source)
#             winsound.PlaySound(r"static\sound clips\End.wav",winsound.SND_ASYNC)
#         self.listenButton.config(command=lambda: Thread(target=self.makeTrue).start())
#         try:
